...
//					device.peripheral.discoverCharacteristics( [CBUUID(string:UART.UUID.write), CBUUID(string:UART.UUID.read), CBUUID(string:UART.UUID.notify)], forService:device.service )
...
//				if let c:CBCharacteristic = characteristic {
...
//					} else if c.UUID.isEqual( CBUUID(string:UART.UUID.notify) ) {
//						UART.logger?.debug?( "Found Notify characteristic" )
//						device.notify = characteristic
//						/// データ更新通知の受け取りを開始する
//						device.peripheral.setNotifyValue( true, forCharacteristic:device.notify )
...
//				}
	...
	func peripheral( _ peripheral:CBPeripheral, didUpdateValueFor characteristic:CBCharacteristic, error:Error? )
	{
		...
		// LOG( "Received data on a characteristic: \(peripheral)" )
...
//						UART.logger?.verbose?( "RX:\(out.description)" )
//					}
				}
...
//                    // LOG( "\(bytes[i])" )
//                    hwRevision = hwRevision.appendingFormat( "0x%02x, ", bytes[i] )
//                }
//                let string = hwRevision.substring( to: hwRevision.characters.index(hwRevision.endIndex, offsetBy: 2) )
//
//                print( "RX:\(string)" )
//
//                for ( _, value ) in self.delegates {
//                    value.didReadHardwareRevisionString?( device.uuid, data:string )
//                }
//            }
		}
	}
...
extension UARTDeviceDelegate:CBCentralManagerDelegate
{
	...
	{
        UART.logger?.info?( "didDiscover:\(central.state.name)" )
